			...
			try {
				...
				if(!(status.equals("SUCCESS_PAYMENT"))) {
					...
					try {
						...
						else { session.setAttribute("alertMsg", "결제 취소 도중 실패하였습니다."); response.sendRedirect(request.getContextPath() + "/adminList.resv?currentPage=" + currentPage); }
					}
					catch (Exception e) { 
